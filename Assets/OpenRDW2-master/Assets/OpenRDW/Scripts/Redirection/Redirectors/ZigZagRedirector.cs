...
public class ZigZagRedirector : Redirector
{
    ...
    void Initialize()
    {
        ...
        {
            point0 = Utilities.FlattenedPos3D(veWaypoints[0].position);
            point1 = Utilities.FlattenedPos3D(veWaypoints[1].position);
        }
        else
        {
            point0 = Utilities.UnFlatten(waypoints[0]);
            point1 = Utilities.UnFlatten(waypoints[1]);
        }
    ...
    void UpdateWaypoint()
    {
        ...
        if (redirectionManager.movementManager.pathSeedChoice == GlobalConfiguration.PathSeedChoice.VEPath)//jon: newly added VEPath
        {
            userIsNearTarget = Utilities.FlattenedPos3D(redirectionManager.currPos - Utilities.FlattenedPos3D(veWaypoints[waypointIndex].position)).magnitude < WAYPOINT_UPDATE_DISTANCE;
        }
        else
        {
            userIsNearTarget = Utilities.FlattenedPos3D(redirectionManager.currPos - Utilities.UnFlatten(waypoints[waypointIndex])).magnitude < WAYPOINT_UPDATE_DISTANCE;
        }
    ...
    public override void InjectRedirection()
    {
        ...
        if (redirectionManager.movementManager.pathSeedChoice == GlobalConfiguration.PathSeedChoice.VEPath)//jon: newly added VEPath
        {
            virtualTargetPosition = Utilities.FlattenedPos3D(veWaypoints[waypointIndex].position);
        }
        else
        {
        ...
        float minimumRealTranslationRemaining = userToVirtualTarget.magnitude / (redirectionManager.globalConfiguration.MAX_TRANS_GAIN);
        ...
        float expectedRotationFromRotationGain = Mathf.Sign(requiredAngleInjection) * Mathf.Min(Mathf.Abs(requiredAngleInjection), Mathf.Abs(minimumRealRotationRemaining * (redirectionManager.globalConfiguration.MIN_ROT_GAIN - 1)));
        ...
        g_t = g_t > 0 ? Mathf.Min(g_t, redirectionManager.globalConfiguration.MAX_TRANS_GAIN - 1) : Mathf.Max(g_t, redirectionManager.globalConfiguration.MIN_TRANS_GAIN - 1);
        g_r = g_r > 0 ? Mathf.Min(g_r, redirectionManager.globalConfiguration.MAX_ROT_GAIN - 1) : Mathf.Max(g_r, redirectionManager.globalConfiguration.MIN_ROT_GAIN - 1);
