...
public class DeepLearning_Redirector : Redirector
{
    ...
    public override void InjectRedirection()
    {
        ...
        {
            float boxWidth = 0;
            if (globalConfiguration.trackingSpaceChoice.Equals(GlobalConfiguration.TrackingSpaceChoice.Square))
            {
                boxWidth = globalConfiguration.squareWidth;
            }
            else
            {
        ...
        if (states == null || redirectionManager.EndResetCountDown > 0)
        {
