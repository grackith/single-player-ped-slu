...
// SFR2G means modified step-forword reset-to-gradient
public class SFR2G_Resetter : Resetter
{
    ...
    float speedRatio;
    const float STEP_LENGTH = 0.1f; // the length of the step we go forward along the gradient
    const int STEPS = 5;
    ...
    public override void InitializeReset()
    {
        ...
        var stepForwardPos = currPos;
        for (int k = 0; k < STEPS; k++)
        {
            Vector2 tmpDir = getGradientForceByThomasAPF(stepForwardPos);
            stepForwardPos += tmpDir * STEP_LENGTH;
        }

        targetPos = DecideResetPosition(currPos);
        targetDir = getGradientForceByThomasAPF(stepForwardPos);
