...
public class ResearcherUIManager : MonoBehaviour
{
    ...
    [Tooltip("The controller transform to follow (usually right hand)")]
    public Transform controllerTransformToFollow;
    ...
    private void Start()
    {
        ...
        if (attachToController && controllerTransformToFollow != null)
        {
            // Detach from current parent
            researcherPanel.transform.SetParent(null, true);

            // Attach to controller
            researcherPanel.transform.SetParent(controllerTransformToFollow, false);
            ...
            Debug.Log($"ResearcherUIManager: Panel attached to controller {controllerTransformToFollow.name}");
        }
