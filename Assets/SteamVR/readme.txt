...
Requirements:
 ...
 We strongly recommend you opt-in to the SteamVR Beta to make sure   application will work with future versions of SteamVR. Right-click SteamVR inside steam, click Properties, the beta tab, then select the beta branch.
...
Input and Steam:
 ...
 If you publish   game to steam you can let users change their input bindings while the game is not running by setting the location of   action manifest. On the steamworks partner site go to the Application settings, and the Virtual Reality Section. At the bottom you'll see a radio button to designate   title as a SteamVR Input application. You then can set the location of   action manifest. In older versions of the plugin this was next to the executable. In versions 2.3.3 and above this is in [GameName]_Data/StreamingAssets/SteamVR/actions.json.
 ...
 * Fixes the Open Binding UI button. This should now work on all computers as it opens the window in SteamVR instead of through   default browser.
 ...
 * Added C# events with autocomplete to all actions. booleanAction[source].onStateDown +=  Method will auto-generate a method with named variables!
 ...
 * Major Breaking Change: Actions and ActionSets are no longer Scriptable Objects. Make sure to delete   existing SteamVR_Input folder with all   generated stuff in it. This means you will have to reset all Actions and Action Sets in all prefabs and scenes.
 ...
 * Added ability to create partial input bindings to be used in plugins. Create a partial binding folder and include it in   plugin. When users import   plugin they will be presented with the option to import   actions and bindings.
...
* SteamVR_Input_References has been moved to the generated folder so new plugin updates don't wipe  s. Prior beta users: Please delete   existing one in Assets/SteamVR/Resources/SteamVR_Input_References.asset
...
To work around this in the meantime, you will need to manually add the SteamVR_UpdatePoses component to   main camera.
...
* Removed OnGUI and associated help text (i.e. "You may now put on   headset." notification) as this was causing unnecessary overhead.
...
* Added SteamVR_ExternalCamera for filming mixed reality videos.  Automatically enabled when externalcamera.cfg is added to the root of   projects (next to Assets or executable), and toggled by the presence of a third controller.
...
The easiest and safest thing to do is to delete   SteamVR folder, and any files and folders in  
Plugins directory called 'openvr_api', 'steam_api' or 'steam_unity' (and variants).  Additionally, verify there
are no SteamVR files found in Assets/Editor.  Then import the new unitypackage into   project.
...
added to any object in   scene.  If you wish to use it in some scenes, but not others, it is good practice
to add the component to each of   scenes and ensure its Texture is set to None in those that you do not wish
it rendered in.
...
up and running quickly and easily.  You are encouraged to modify these to suit   project's unique needs,
...
Assets/SteamVR/Scripts/SteamVR_Camera.cs - Adds VR support to   existing camera object.
...
[Status] - The prefab is for demonstration purposes only.  It adds an escape menu to   scene.
...
keep it from rendering other elements of   scene, you should create a unique layer used by all of   gui
...
same texture as it may clear   content.
...
By manually adding a GameObject with the SteamVR_Render component on it to   scene, you can specify a left
...
SteamVR_Events.<EventType>.Listen.  Be sure to remove   handler when no longer needed.
...
If you are releasing   game on Steam (i.e. have a Steam ID and are calling Steam_Init through the
...
* HmdError_Init_HmdNotFound - SteamVR cannot detect   VR headset, ensure the USB cable is plugged in.
If that doesn't work, try deleting   Steam/config/steamvr.cfg.

* HmdError_Init_InterfaceNotFound - Make sure   SteamVR runtime is up to date.
...
set the environement variable ForceDXGICreateFactory1 = 1 and launch the Unity Editor or   standalone builds
