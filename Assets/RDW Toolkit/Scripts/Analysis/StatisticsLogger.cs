...
public class StatisticsLogger : MonoBehaviour {
    ...
    void Awake()
    {
        // Modify path for Android (Quest) platform
        if (Application.platform == RuntimePlatform.Android)
        {
            RESULT_DIRECTORY = Application.persistentDataPath + "/RDWT_Results/";
        }
        else
        {
    ...
    // Override the original method to just store data without writing files
    public void LogExperimentSummaryStatisticsResultsSCSV(List<Dictionary<string, string>> experimentResults)
    {
        // Just store the data without writing to file
        this.experimentResults = experimentResults;

        // Find and send data to VRResearchDataCollector if available
        VRResearchDataCollector dataCollector = FindObjectOfType<VRResearchDataCollector>();
        if (dataCollector != null)
        {
            SendDataToVRResearchLogger(dataCollector);
        }
    }
    ...
    public void SendDataToVRResearchLogger(VRResearchDataCollector dataCollector)
    {
        if (dataCollector == null || state != LoggingState.complete)
            return;

        // Create a folder for RDWT data within the VRResearchDataCollector's structure
        string rdwtFolderPath = Path.Combine(
            Application.persistentDataPath,
            "VRResearch",
            dataCollector.participantID,
            "RDWT_Data");

        Directory.CreateDirectory(rdwtFolderPath);

        // Save RDWT data to CSV files in the same format VRResearchDataCollector uses
        SaveRDWTSummaryData(rdwtFolderPath);

        if (logSampleVariables)
        {
            SaveRDWTSampledData(rdwtFolderPath);
        }

        Debug.Log("RDWT data sent to VRResearchDataCollector folder: " + rdwtFolderPath);
    }

    private void SaveRDWTSummaryData(string folderPath)
    {
        // Get summary statistics
        Dictionary<string, string> summaryStats = GetExperimentResultForSummaryStatistics(
        ...
        // Save the file
        string filePath = Path.Combine(folderPath, "rdwt_summary_stats.csv");
        File.WriteAllText(filePath, csvBuilder.ToString());
    }

    private void SaveRDWTSampledData(string folderPath)
    {
        Dictionary<string, List<float>> oneDimensionalSamples;
        Dictionary<string, List<Vector2>> twoDimensionalSamples;
        GetExperimentResultsForSampledVariables(out oneDimensionalSamples, out twoDimensionalSamples);

        // Save 1D data
        foreach (var entry in oneDimensionalSamples)
        {
            StringBuilder csvBuilder = new StringBuilder();
            csvBuilder.AppendLine("Timestamp,Value");

            float timeStep = 1.0f / samplingFrequency;
            float currentTime = 0;

            foreach (var value in entry.Value)
            {
                csvBuilder.AppendLine($"{currentTime},{value}");
                currentTime += timeStep;
            }

            string filePath = Path.Combine(folderPath, $"rdwt_1d_{entry.Key}.csv");
            File.WriteAllText(filePath, csvBuilder.ToString());
        }

        // Save 2D data
        foreach (var entry in twoDimensionalSamples)
        {
            StringBuilder csvBuilder = new StringBuilder();
            csvBuilder.AppendLine("Timestamp,X,Y");
            ...
            foreach (var value in entry.Value)
            {
                csvBuilder.AppendLine($"{currentTime},{value.x},{value.y}");
                currentTime += timeStep;
            }

            string filePath = Path.Combine(folderPath, $"rdwt_2d_{entry.Key}.csv");
            File.WriteAllText(filePath, csvBuilder.ToString());
        }
    }
}
